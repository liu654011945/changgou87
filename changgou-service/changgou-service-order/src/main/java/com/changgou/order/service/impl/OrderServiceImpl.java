...
public class OrderServiceImpl implements OrderService {
    ...
    public void add(Order order) {
        ...
        List<OrderItem> orderItems = cartService.list(order.getUsername());//当前的登录的用户的名称
        ...
        for (OrderItem orderItem : orderItems) {
            num += orderItem.getNum();//购买数量
            totalMoney += orderItem.getMoney();//总金额
            ...
            //4.减少库存 调用feign
            skuFeign.decCount(orderItem.getSkuId(),orderItem.getNum());//思考 优化下
        }
        //设置数量合计
