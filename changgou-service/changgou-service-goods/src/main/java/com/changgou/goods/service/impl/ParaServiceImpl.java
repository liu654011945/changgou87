...
public class ParaServiceImpl implements ParaService {
    ...
    public List<Para> findByCategoryId(Integer id) {
        //1.先根据商品分类的ID 获取商品分类的对象
        Category category = categoryMapper.selectByPrimaryKey(id);
        //2.再获取商品分类对象中template_id的值
        Integer templateId = category.getTemplateId();
        //3.根据template_id的条件来查询出参数列表
        Para condition = new Para();//条件只能针对 =
        condition.setTemplateId(templateId);
        return paraMapper.select(condition);//根据条件查询 select * from tb_para where template_id=?
    }
}
