...
public class AddressController {
    ...
    /**
     * 根据当前登录的用户的用户名查询该用户下的所有的地址列表
     */
    @GetMapping(value = "/user/list")
    public Result<List<Address>> list() {
        //1.需要获取当前登录的用户名（从token中解析出来）
        String username = tokenDecode.getUsername();//获取到用户名
        //2.调用service 从书库中查询出该用户下的所有的地址列表
        List<Address> addressList = addressService.list(username);
        //3.返回
        return new Result<List<Address>>(true,StatusCode.OK,"查询地址列表成功",addressList);
    }
